...
import numpy as np
from SPM import ShufflePatchMix
...
def get_augmentation(aug_type, alpha=2.0, beta=2.0, patch_height=28, mix_prob=0.8, normalize=None):
    if not normalize:
    ...
    if aug_type == "moco-v2":
        ...
        return transforms.Compose(
            [
                transforms.Resize((256, 256)),
                transforms.CenterCrop(224),
                ShufflePatchMix(patch_height=patch_height, patch_width=patch_height, mix_prob=1.0, alpha=alpha, beta=beta),
                transforms.ToTensor(),
                normalize,
            ]
        )
